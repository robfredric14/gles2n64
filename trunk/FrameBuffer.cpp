...
{
    LOG("FrameBuffer_RemoveBuffer(%d) \n", address);
...
{
    LOG("FrameBuffer_SaveBuffer(%d, %d, %d, %d) \n", address, size, width, height);
    ...
    current->texture->width = (unsigned int)(current->width * OGL.scaleX);
    current->texture->height = (unsigned int)(current->height * OGL.scaleY);
    ...
    current->texture->realWidth = pow2( (unsigned int)(current->width * OGL.scaleX) );
    current->texture->realHeight = pow2( (unsigned int)(current->height * OGL.scaleY) );
...
{
    LOG("FrameBuffer_RenderBuffer(%d) \n", address);
...
{
    LOG("FrameBuffer_RestoreBuffer(%d, %d, %d) \n", address, size, width);
...
{
    LOG("FrameBuffer_ActivateBufferTexture(%d, %d) \n", t, buffer);
